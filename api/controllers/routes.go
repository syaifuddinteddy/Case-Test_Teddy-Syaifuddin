...
import (
	"gitlab.com/syaifuddin.teddy/test-case-majoo/api/middlewares"
)
...
func (s *Server) initializeRoutes() {
	...
	s.Router.HandleFunc("/img", middlewares.SetMiddlewareAuthentication(s.UploadImages)).Methods("POST")
	s.Router.HandleFunc("/img/{id}", middlewares.SetMiddlewareAuthentication(s.ShowImages)).Methods("GET")

	s.Router.HandleFunc("/sudoku/solver", middlewares.SetMiddlewareJSON(s.SudokuSolver)).Methods("POST")
}
